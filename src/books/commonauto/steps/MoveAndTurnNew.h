...
class MoveAndTurnNew : public AutoStep {

    public:
        MoveAndTurnNew(const double x, const double y, const double angle, const double timeToDrive, const bool hold, const bool jerk, const double speed) 
            : AutoStep("MoveAndTurnNew"), m_x(x), m_y(y), m_targetAngle(angle), m_totalTimeToDrive(timeToDrive), m_hold(hold), m_jerk(jerk), m_speed(speed) {}
        ...
        bool Execute() {
            ...
            // drive the robot
            SwerveTrain::GetInstance().Drive(m_x, m_y, turnSpeed, m_hold, (turnSpeed == 0), m_speed);
