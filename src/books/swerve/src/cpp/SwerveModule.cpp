...
SwerveModule::SwerveModule(const int &canDriveID, const int &canSwerveID, const int &canEncoderID) {
    ...
    frc::SmartDashboard::PutNumber("Position", m_swerveMotorEncoder->GetAbsolutePosition());
    //m_swerveZeroPosition = m_swerveMotorEncoder->GetAbsolutePosition();
...
frc::SwerveModuleState& SwerveModule::GetState() {
    ...
    state.speed = GetDriveVelocity();
    state.angle = units::degree_t(GetSwervePosition() / R_nicsConstant * 180.0);
    return state;
}
